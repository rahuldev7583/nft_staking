...
impl <'info> InitializeConfig <'info>{
    pub fn initialize_config(&mut self, points_per_stake: u8, max_stake: u8, freeze_period: u32, bumps: &InitializeConfigBumps)-> Result<()>{
     self.config.set_inner(StakeConfig { points_per_stake: points_per_stake, max_stake: max_stake, freeze_period: freeze_period, rewards_bump: bumps.rewards_mint, bump: bumps.config}); 
